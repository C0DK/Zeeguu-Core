...
if __name__ == "__main__":
    ...
    description = input(
        f'Description (Enter for: {test_feed.description}): ') or test_feed.description
    print(f'= {description}')

    _language = input("Language code (e.g. en): ")
    print(f'= {_language}')

    feed_url = Url.find_or_create(db.session, _feed_url)
    language = Language.find_or_create(_language)

    rss_feed = RSSFeed.find_or_create(db.session,
                                      feed_url,
                                      feed_name,
                                      description,
                                      icon_name=icon_name,
